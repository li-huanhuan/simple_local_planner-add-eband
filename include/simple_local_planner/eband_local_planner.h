#ifndef EBAND_LOCAL_PLANNER_H
#define EBAND_LOCAL_PLANNER_H
...
namespace simple_local_planner{
  ...
  class EBandPlanner{
    ...
    public:
       ...
       */
      void reconfigure(SimpleLocalPlannerConfig& config);
    ...
    private:
      ...
      double costmap_weight_; // the costmap weight or scaling factor 成本图的权重或比例因子。
       ...
       */
      bool applyForces(size_t bubble_num, std::vector<Bubble>& band, std::vector<geometry_msgs::WrenchStamped> forces);
       ...
       */
      bool moveApproximateEquilibrium(const size_t& bubble_num, const std::vector<Bubble>& band, Bubble& curr_bubble,
       ...
       */
      bool getForcesAt(size_t bubble_num, std::vector<Bubble> band, Bubble curr_bubble, geometry_msgs::WrenchStamped& forces);
       ...
       * @return true if forces were calculated successfully
       */
      bool calcInternalForces(size_t bubble_num, std::vector<Bubble> band, Bubble curr_bubble, geometry_msgs::WrenchStamped& forces);


      /**
       * @brief Calculates external forces for bubbles along the band [depends shape, environment]
       * 计算沿松紧带的气泡的外力[取决于形状，环境]。
       * @param position in band for which internal forces shall be calculated
       * @param Bubble for which external forces shall be calculated
       * @param reference to variable via which forces and torques are given back
       * @return true if forces were calculated successfully
       */
      bool calcExternalForces(size_t bubble_num, Bubble curr_bubble, geometry_msgs::WrenchStamped& forces);
       ...
       * @return true if forces were calculated successfully
       */
      bool suppressTangentialForces(size_t bubble_num, std::vector<Bubble> band, geometry_msgs::WrenchStamped& forces);
       ...
       * 这会将计划从一系列姿势转换成一个带状，一系列气泡。 因此，它可以计算到周围障碍物的距离，并得出气泡的膨胀或半径。
