...
namespace simple_local_planner{
  ...
  EBandPlanner::EBandPlanner() : costmap_ros_(nullptr), initialized_(false) {}
  ...
  void EBandPlanner::reconfigure(simple_local_planner::SimpleLocalPlannerConfig& config)
  {
  ...
  bool EBandPlanner::setPlan(const std::vector<geometry_msgs::PoseStamped>& global_plan)
  {
    ...
    {
      ROS_ERROR("Attempt to pass empty path to optimization. Valid path needs to have at least 2 Frames. This one has %d.", static_cast<int>(global_plan.size()));
      return false;
    }
  ...
  bool EBandPlanner::addFrames(const std::vector<geometry_msgs::PoseStamped>& plan_to_add, const AddAtPosition& add_frames_at)
  {
    ...
    if(add_frames_at == add_front)
    {
      ...
      for(int i = (static_cast<int>( elastic_band_.size() ) - 1); i >= 0; i--)
      {
        ...
        if(checkOverlap(band_to_add.back(), elastic_band_.at(static_cast<size_t>(i))))
        {
          bubble_connect = i;
          connected = true;
          break;
    ...
    else
    {
      ...
      for(size_t i = 0; i < (elastic_band_.size() - 1); i++)
      {
        ...
        {
          bubble_connect = static_cast<int>(i);
          connected = true;
          break;
        }
      }
    ...
    if(add_frames_at == add_front)
    {
      ...
      tmp_iter1 = tmp_band.begin() + static_cast<int>( band_to_add.size() ) - 1;
    ...
    else
    {
      ...
      // and get iterators to connecting bubbles
      tmp_iter1 = tmp_band.begin() + static_cast<int>( elastic_band_.size() ) - 1;
      tmp_iter2 = tmp_iter1 + 1;
    }
  ...
  bool EBandPlanner::optimizeBand(std::vector<Bubble>& band)
  {
    ...
    {
      ROS_WARN("Elastic Band expects plan for optimization in the %s frame, the plan was sent in the %s frame.",
    ...
    for(size_t i = 0; i < band.size(); i++)
    {
      ...
      {
        // 优化（弹性带）-距离计算失败。 ％d的第％d帧可能在地图坐标之外。
        ROS_WARN("Optimization (Elastic Band) - Calculation of Distance failed. Frame %d of %d Probably outside map coordinates.",
                  static_cast<int>(i), static_cast<int>( band.size() ) );
        return false;
      }
      ...
      if(distance == 0.0)
      {
        ...
        // 框架不得立即发生碰撞->否则，梯度的计算将在以后无效。
        ROS_WARN("Optimization (Elastic Band) - Calculation of Distance failed. Frame %d of %d in collision. Plan invalid. Trying to refine band.",
            static_cast<int>(i), static_cast<int>( band.size() )  );
    ...
    if(!refineBand(band))
    {
      ROS_WARN("Elastic Band is broken. Could not close gaps in band. Global replanning needed.");
      return false;
    }
    ...
    for(int i = 0; i < num_optim_iterations_; i++)
    {
      ...
      {
        ROS_WARN("Optimization failed while trying to modify Band.");
      ...
      {
        ROS_WARN("Optimization failed while trying to refine modified band");
  ...
  bool EBandPlanner::refineBand(std::vector<Bubble>& band)
  {
    ...
    if(band.size() < 2)
    {
      ROS_WARN("Attempt to convert empty band to plan. Valid band needs to have at least 2 Frames. This one has %d.", ( static_cast<int>(band.size()) ) );
      return false;
    }
  ...
  bool EBandPlanner::removeAndFill(std::vector<Bubble>& band, std::vector<Bubble>::iterator& start_iter,std::vector<Bubble>::iterator& end_iter)
  {
    ...
    if(overlap)
    {
      ...
      // 如果开始和结束之间有气泡，则将其删除（它们是多余的，因为开始和结束确实重叠）。
    ...
    // 如果气泡不重叠->检查开始和结束之间是否仍然有气泡。
    if((start_iter + 1) < end_iter)
    {
      // split remaining sequence of bubbles
      // get distance between start and end iterator for this intervall
      // 拆分剩余的气泡序列可获取此时间间隔的开始迭代器和结束迭代器之间的距离。
      mid_int = static_cast<int>( std::distance(start_iter, end_iter) ); //std::distance() 函数用于计算两个迭代器表示的范围内包含元素的个数
      mid_int = mid_int/2; // division by integer implies floor (round down) 除以整数意味着下限（向下舍入）。

      // now get iterator pointing to the middle (roughly)
      tmp_iter = start_iter + mid_int;
      // and realative position of end_iter to tmp_iter
      // 和end_iter与tmp_iter的相对位置。
      diff_int = static_cast<int>( std::distance(tmp_iter, end_iter) );
      ...
      diff_int = static_cast<int>( std::distance(start_iter, tmp_iter) );
      ...
      if(checkOverlap(*(tmp_iter-1), *(tmp_iter+1)))
      {
        ...
        diff_int = static_cast<int>( std::distance((tmp_iter + 1), end_iter) );
  ...
  bool EBandPlanner::fillGap(std::vector<Bubble>& band, std::vector<Bubble>::iterator& start_iter,std::vector<Bubble>::iterator& end_iter)
  {
    ...
    if(!interpolateBubbles(start_iter->center, end_iter->center, interpolated_center))
    {
      ...
      start_num = static_cast<int>( std::distance(band.begin(), start_iter) );
      end_num = static_cast<int>( std::distance(band.begin(), end_iter) );
  ...
  bool EBandPlanner::modifyBandArtificialForce(std::vector<Bubble>& band)
  {
    ...
    while( (i>0) && (i < (band.size() - 1 ) ) )
    {
      ...
      if(!calcExternalForces(i, band.at( i ), external_forces.at( i ) ) )
      {
        ...
        ROS_WARN("Calculation of external forces failed");
        return false;
      }
      ...
      if(!suppressTangentialForces(i, band, forces.at(i)))
      {
        ...
        ROS_WARN("Supression of tangential forces failed");
        return false;
      }
      ...
      {
        // band invalid
        ROS_WARN("Band is invalid - Stopping Modification");
        return false;
      }
  ...
  bool EBandPlanner::applyForces(size_t bubble_num, std::vector<Bubble>& band, std::vector<geometry_msgs::WrenchStamped> forces)
  {
    ...
    {
      ROS_DEBUG("Calculation of Distance failed. Frame %d of %d Probably outside map. Discarding Changes", static_cast<int>(bubble_num), static_cast<int>( band.size() ) );
    ...
    {
      // frame must not be immediately in collision -> otherwise calculation of gradient will later be invalid
      ROS_DEBUG("Calculation of Distance failed. Frame %d of %d in collision. Plan invalid. Discarding Changes", static_cast<int>(bubble_num), static_cast<int>( band.size() ) );
      // this bubble must not be changed, but band is still valid -> continue with other bubbles
      return true;
    }
    ...
    if(!getForcesAt(bubble_num, band, new_bubble, new_bubble_force))
    {
      ...
      ROS_DEBUG("Cannot calculate forces on bubble %d at new position - discarding changes", static_cast<int>( bubble_num ) );
      return true;
    }
    ...
    {
      ROS_DEBUG("Detected zero-crossings in force on bubble %d. Checking total change in force.", static_cast<int>( bubble_num ));
      ...
      {
        ROS_DEBUG("Detected significante change in force (%f to %f) on bubble %d. Entering Recursive Approximation.", abs_old_force, abs_new_force, static_cast<int>( bubble_num ));
    ...
    start_iter = start_iter + static_cast<int>(bubble_num) - 1;
    ...
    start_iter = start_iter + static_cast<int>(bubble_num);
    end_iter = start_iter + 1;
  ...
  bool EBandPlanner::moveApproximateEquilibrium(const size_t& bubble_num, const std::vector<Bubble>& band, Bubble& curr_bubble,
  ...
  bool EBandPlanner::getForcesAt(size_t bubble_num, std::vector<Bubble> band, Bubble curr_bubble, geometry_msgs::WrenchStamped& forces)
  {
  ...
  bool EBandPlanner::calcInternalForces(size_t bubble_num, std::vector<Bubble> band, Bubble curr_bubble, geometry_msgs::WrenchStamped& forces)
  {
    // check if plugin initialized
    if(!initialized_)
    {
    ...
    ROS_ASSERT( bubble_num < (band.size() - 1) );
  ...
  bool EBandPlanner::calcExternalForces(size_t bubble_num, Bubble curr_bubble, geometry_msgs::WrenchStamped& forces)
  {
    // check if plugin initialized
    if(!initialized_)
    {
    ...
    {
      ROS_DEBUG("Bubble %d probably at edge of map - cannot retrieve distance information to calculate external forces", static_cast<int>( bubble_num ));
  ...
  bool EBandPlanner::suppressTangentialForces(size_t bubble_num, std::vector<Bubble> band, geometry_msgs::WrenchStamped& forces)
  {
    //cycle over all bubbles except first and last (these are fixed)
    if(band.size() <= 2)
    {
  ...
  bool EBandPlanner::calcObstacleKinematicDistance(geometry_msgs::Pose center_pose, double& distance)
  {
    ...
    {
      if (disc_cost == costmap_2d::FREE_SPACE)
      ...
      distance = -log(factor) / weight; //disc_cost = 1时，distance = 0.15m
    }
  ...
  bool EBandPlanner::convertPlanToBand(std::vector<geometry_msgs::PoseStamped> plan, std::vector<Bubble>& band)
  {
    ...
    for(size_t i = 0; i < plan.size(); i++)
    {
      ...
      // calc Size of Bubbles by calculating Dist to nearest Obstacle [depends kinematic, environment]
      // 通过计算距离最近的障碍物来计算气泡的大小[取决于运动学，环境]。
      if(!calcObstacleKinematicDistance(tmp_band[i].center.pose, distance))
      {
        // frame must not be immediately in collision -> otherwise calculation of gradient will later be invalid
        // 框架不得立即发生碰撞->否则，梯度的计算将在以后无效。
        ROS_WARN("Calculation of Distance between bubble and nearest obstacle failed. Frame %d of %d outside map", static_cast<int>(i), ( static_cast<int>( plan.size() ) ) );
        return false;
      }
      ...
      if(distance <= 0.0)
      {
        // frame must not be immediately in collision -> otherwise calculation of gradient will later be invalid
        // 框架不得立即发生碰撞->否则，梯度的计算将在以后无效。
        ROS_WARN("Calculation of Distance between bubble and nearest obstacle failed. Frame %d of %d in collision. Plan invalid", static_cast<int>(i), static_cast<int>( plan.size() ));
        // TODO if frame in collision try to repair band instaed of aborting averything
        // 如果框架在碰撞中尝试修复带子而不是放弃所有东西，则必须执行。
        return false;
      }
